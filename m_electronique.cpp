...
{QSqlQuery query;
  ...
  query.prepare("insert into MATERIELS (refs,type_e,prix,voltage) values (:refs,:type_e,:prix,:voltage)");
...
{ QSqlQuery query;
   ...
   query.prepare("delete from MATERIELS where refs=:refs");
   query.bindValue(":refs",res);
  return query.exec();}
...
{ QSqlQuery query;
   ...
   query.prepare("delete from MATERIELS ");
...
{ QSqlQueryModel * model=new QSqlQueryModel();
    model->setQuery("select * from MATERIELS ORDER BY type_e ASC");// asc assendant dsc descendant
    model->setHeaderData(0,Qt::Horizontal,QObject::tr("reference"));
        model->setHeaderData(1,Qt::Horizontal,QObject::tr("type"));
         model->setHeaderData(2,Qt::Horizontal,QObject::tr("prix"));
         model->setHeaderData(3,Qt::Horizontal,QObject::tr("voltage"));
...
{ {QSqlQuery qry;
         ...
         qry.prepare("select * from MATERIELS where refs like'%"+a+"%' OR type_e like '%"+a+"%' OR prix like '%"+a+"%' OR voltage like '%"+a+"%'");
                     qry.exec();
                 m->setQuery(qry);
         g->setModel(m);
...
{QSqlQuery query ;
    query.prepare("update MATERIELS set refs=?,type_e=?,prix=?,voltage=? where refs=?");
...
{   QSqlQuery q;
    int i=0;
    q.exec("select refs from MATERIELS");
    while (q.next()) {
